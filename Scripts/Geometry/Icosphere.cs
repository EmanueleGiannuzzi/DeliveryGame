...
public static class Icosphere {
	// Indices of the vertex pairs that make up each of the initial 12 edges
		...
		public static ArrayMesh GenerateMesh(int resolution, float radius = 1) {
			...
			for (int i = 0; i < vertices.items.Length; i++) {
				normals[i] = vertices.items[i].Normalized();

				// UV calculation
				Vector3 normalized = vertices.items[i].Normalized();
			...
			ArrayMesh meshData = new ArrayMesh(); //= new SimpleMeshData(vertices.items, triangles.items, normals, name: "Ico Sphere");
			
			var surfaceArray = new Godot.Collections.Array();
			surfaceArray.Resize((int)Mesh.ArrayType.Max);
			surfaceArray[(int)Mesh.ArrayType.Vertex] = vertices.items;
			surfaceArray[(int)Mesh.ArrayType.TexUV] = uvs.ToArray();
			surfaceArray[(int)Mesh.ArrayType.Normal] = normals;
			surfaceArray[(int)Mesh.ArrayType.Index] = triangles.items;
			meshData.AddSurfaceFromArrays(Mesh.PrimitiveType.Triangles, surfaceArray);
			return meshData;
