...
public partial class CubeSphereGenerator : Node {
	...
	private void updateMesh() {
		...
		var randomColor = new Color(GD.Randf(), GD.Randf(), GD.Randf(), 1);
		...
		foreach (Vector3 faceNormal in faceNormals)
		{
			for (int y = 0; y < numSubdivisions; y++)
			{
				for (int x = 0; x < numSubdivisions; x++)
				{
					...
					addFaceToMeshGen(resolution, faceNormal, startT, endT, radius);
	...
	private void addFaceToMeshGen(int resolution, Vector3 normal, Vector2 startT, Vector2 endT, float radius) {
